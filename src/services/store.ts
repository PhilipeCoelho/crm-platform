...
export function useCRMStore(): CRMStore {
    ...
    const [pipelines, setPipelines] = useState<Record<string, Pipeline>>(DEFAULT_PIPELINES);
...
const addStage = async (pipelineId: string, title: string) => {
    const currentStages = pipelines[pipelineId]?.stages || [];
    const newOrderIndex = currentStages.length;
