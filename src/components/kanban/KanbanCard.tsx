...
function DealCard({ deal, currency, onPreview }: Props) {
    ...
    if (isDragging) return <div ref={setNodeRef} style={style} className="bg-background p-3 rounded-lg border-2 border-primary opacity-30 h-[100px]" />;
    ...
    return (
        <div
            ...
            {...listeners}
            onClick={handleClick}
            className={`group relative bg-card/80 backdrop-blur-sm p-3 rounded-md shadow-[0_1px_2px_rgba(0,0,0,0.05)] hover:shadow-[0_6px_20px_rgba(0,0,0,0.15)] dark:hover:shadow-[0_6px_20px_rgba(0,0,0,0.35)] transition-all duration-200 ease-out cursor-pointer touch-none select-none border border-[color:var(--border)] hover:-translate-y-[2px] hover:border-primary/50
