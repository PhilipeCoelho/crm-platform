        ...
        return (
            <div className="w-full bg-[#FFFFFF] dark:bg-[#0D0D0D] py-12 px-6 border-b border-[#E5E7EB] dark:border-[#1F1F1F] relative">
                ...
                <div className="max-w-[1200px] mx-auto flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm text-[#6B7280] dark:text-[#A1A1A1] mt-2 max-w-md">
...
function RecommendationCard({ rec }: { rec: StrategicRecommendation }) {
    ...
    const getImpactStyle = (impacto: string) => {
        switch (impacto) {
            case 'alto': return {
                ...
                badge: 'bg-rose-100 text-rose-800 dark:bg-[#EF4444]/15 dark:text-[#EF4444]'
            };
            case 'medio': return {
                border: 'border-l-[#F59E0B]',
                badge: 'bg-amber-100 text-amber-800 dark:bg-[#F59E0B]/15 dark:text-[#F59E0B]'
            };
            case 'baixo': return {
                border: 'border-l-[#22C55E]',
                badge: 'bg-emerald-100 text-emerald-800 dark:bg-[#22C55E]/15 dark:text-[#22C55E]'
            };
            default: return {
                border: 'border-l-gray-500',
                badge: 'bg-gray-100 text-gray-800 dark:bg-muted dark:text-[#8A8A8A]'
            };
        }
    };
    ...
    return (
        <div className={`p-6 bg-[#FFFFFF] dark:bg-[#141414] border border-[#E5E7EB] dark:border-[#262626] border-l-[3px] ${impactStyle.border} shadow-sm rounded-xl flex flex-col gap-4`}>
            ...
            <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h4 className="font-semibold text-[#111827] dark:text-[#EAEAEA] text-lg leading-tight">
                            {rec.titulo}
                        </h4>
                    </div>
                    <p className="text-[#6B7280] dark:text-[#A1A1A1] text-sm leading-relaxed max-w-4xl">
                        {rec.mensagem}
                    </p>
                </div>

                <span className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-md bg-[#F7F9FC] dark:bg-[#1A1A1A] text-[#6B7280] dark:text-[#8A8A8A]">
                    {rec.area}
                </span>
            </div>

            <div className="bg-[#F7F9FC] dark:bg-[#0D0D0D]/50 rounded-md p-4 flex items-start gap-3 mt-2 border border-[#E5E7EB] dark:border-[#262626]/40">
                <Lightbulb className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" />
                <span className="text-sm font-medium text-[#111827] dark:text-[#EAEAEA] leading-relaxed">
