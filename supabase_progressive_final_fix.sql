...
BEGIN
    ...
    IF (v_stage_name LIKE '%LEAD%' AND v_stage_name NOT LIKE '%ENGAJADO%') THEN
        ...
        VALUES (NEW.user_id, NEW.id, 'message', 'WhatsApp: Mensagem inicial', NOW() + interval '1 hour', 'pending', true, 'LEAD', 1);
    
    -- LEAD ENGAJADO
    ELSIF (v_stage_name LIKE '%ENGAJADO%') THEN
        INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
        VALUES (NEW.user_id, NEW.id, 'message', 'Resposta + Pergunta Estratégica', NOW() + interval '1 hour', 'pending', true, 'ENGAJADO', 1);

    -- REUNIÃO DE DIAGNÓSTICO
    ELSIF (v_stage_name LIKE '%DIAGN%STICO%') THEN
        INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
        VALUES (NEW.user_id, NEW.id, 'message', 'RD – Confirmação oficial', NOW() + interval '1 hour', 'pending', true, 'DIAGNOSTICO', 1);

    -- FECHAMENTO
    ELSIF (v_stage_name LIKE '%FECHAMENTO%') THEN
        INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
        VALUES (NEW.user_id, NEW.id, 'message', 'FE – Resumo pós-reunião', NOW() + interval '1 hour', 'pending', true, 'FECHAMENTO', 1);
    END IF;
    ...
    IF (NEW.is_automatic = true AND NEW.status = 'completed' AND (OLD.status IS NULL OR OLD.status != 'completed')) THEN
        ...
        IF (NEW.origin_stage = 'LEAD') THEN
            CASE NEW.sequence_step
                WHEN 1 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'email', 'Email: Apresentação', NOW() + interval '1 day', 'pending', true, 'LEAD', 2);
                WHEN 2 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'call', 'Ligação: Tentativa 1', NOW() + interval '2 days', 'pending', true, 'LEAD', 3);
                WHEN 3 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'analysis', 'Vídeo: Análise digital', NOW() + interval '2 days', 'pending', true, 'LEAD', 4);
                WHEN 4 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'email', 'Email: Insight estratégico', NOW() + interval '3 days', 'pending', true, 'LEAD', 5);
                WHEN 5 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'audit', 'Vídeo: Mini auditoria', NOW() + interval '2 days', 'pending', true, 'LEAD', 6);
                WHEN 6 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'call', 'Ligação: Tentativa 2', NOW() + interval '2 days', 'pending', true, 'LEAD', 7);
                WHEN 7 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'email', 'Email: Convite fechamento', NOW() + interval '3 days', 'pending', true, 'LEAD', 8);
                ELSE NULL;
            END CASE;
        ...
        ELSIF (NEW.origin_stage = 'ENGAJADO') THEN
            CASE NEW.sequence_step
                WHEN 1 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'message', 'Apontar o Problema', NOW() + interval '1 day', 'pending', true, 'ENGAJADO', 2);
                WHEN 2 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
        ...
        ELSIF (NEW.origin_stage = 'DIAGNOSTICO') THEN
            CASE NEW.sequence_step
                WHEN 1 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'task', 'RD – Preparar análise digital', NOW() + interval '4 hours', 'pending', true, 'DIAGNOSTICO', 2);
                WHEN 2 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'message', 'RD – Lembrete 24h', NOW() + interval '1 day', 'pending', true, 'DIAGNOSTICO', 3);
                WHEN 3 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'message', 'RD – Lembrete 1h', NOW() + interval '1 day', 'pending', true, 'DIAGNOSTICO', 4);
                WHEN 4 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'task', 'RD – Reunião realizada', NOW() + interval '1 hour', 'pending', true, 'DIAGNOSTICO', 5);
                ELSE NULL;
            END CASE;
        ...
        ELSIF (NEW.origin_stage = 'FECHAMENTO') THEN
            CASE NEW.sequence_step
                WHEN 1 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'message', 'FE – Follow-up 1', NOW() + interval '2 days', 'pending', true, 'FECHAMENTO', 2);
                WHEN 2 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'message', 'FE – Follow-up 2', NOW() + interval '2 days', 'pending', true, 'FECHAMENTO', 3);
                WHEN 3 THEN INSERT INTO public.activities (user_id, deal_id, type, title, date, status, is_automatic, origin_stage, sequence_step)
                            VALUES (NEW.user_id, NEW.deal_id, 'message', 'FE – Última chamada', NOW() + interval '3 days', 'pending', true, 'FECHAMENTO', 4);
                ELSE NULL;
            END CASE;
        END IF;
    END IF;
