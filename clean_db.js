...
async function run() {
  ...
  // Find Won/Lost deals to show the user what we are deleting
  const { data: unwanted } = await supabase.from('deals').select('id, title, status').neq('status', 'active').neq('status', 'open');
  ...
  const { error: err2 } = await supabase.from('deal_analytics').delete().neq('deal_id', '00000000-0000-0000-0000-000000000000'); // Delete all
